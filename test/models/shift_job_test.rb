...
class ShiftJobTest < ActiveSupport::TestCase
  ...
  context "Creating a context for shift_jobs" do
    ...
    should "identify a non-existent shift as part of an invalid shift_job" do
      ghost_shift = FactoryBot.build(:shift, assignment: @assign_cindy, date: 3.days.from_now.to_date)
      bad_shift_job = FactoryBot.build(:shift_job, shift: ghost_shift, job: @cashier)
      deny bad_shift_job.valid?
    end

    should "identify a non-active or non-existent job as part of an invalid shift job" do
      inactive_job_on_shift = FactoryBot.build(:shift_job, shift: @cindy_shift1, job: @mover)
      deny inactive_job_on_shift.valid?
      ghost_job  = FactoryBot.build(:job, name: "Greeting")
      nonexistent_job_on_shift = FactoryBot.build(:shift_job, shift: @cindy_shift1, job: ghost_job)
