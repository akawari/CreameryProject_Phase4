  ...
  context "Creating a context for shifts" do
    setup do 
      @ed = FactoryBot.create(:employee)
  	  ...
  	  @given_assign = FactoryBot.create(:assignment, store: @cmu, employee: @ed, start_date: 3.months.ago.to_date, end_date: nil)
      create_shifts
    end
    ...
    should "Check if end_time automatically set" do
      @another_shift = FactoryBot.create(:shift, start_time: Date.current + 2.hours)
    ...
    should "have a scope completed that works" do
      @past_shift = FactoryBot.create(:shift)
      @past_shift.update_attribute(:date, Date.current-5)
      @job_cash = FactoryBot.create(:job)
      @shift_job_cash = FactoryBot.create(:shift_job, shift_id: 3, job_id: 1)
    ...
    should "have a scope by_store that works" do
      @akland = FactoryBot.create(:store, name: "Akland", phone: "412-268-8211")
      @cindy = FactoryBot.create(:employee, first_name: "Cindy", last_name: "Crawford", ssn: "084-35-9822", date_of_birth: 17.years.ago.to_date)
      @another_assign = FactoryBot.create(:assignment, store: @akland, employee: @cindy, start_date: 4.months.ago.to_date, end_date: nil)
      @past_shift = FactoryBot.create(:shift, assignment_id: 2)
      @past_shift.update_attribute(:date, Date.current-5)
